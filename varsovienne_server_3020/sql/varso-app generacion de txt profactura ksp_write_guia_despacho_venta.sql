/*
exec ksp_GDV_imprimir 1131835,1097310 ;
*/

IF OBJECT_ID('ksp_GDV_imprimir', 'P') IS NOT NULL DROP PROCEDURE ksp_GDV_imprimir;  
GO  
CREATE PROCEDURE ksp_GDV_imprimir ( @nroint int, @folio int ) 
With Encryption
AS
BEGIN
	--
	...
	if not exists ( select * from ktb_lineas_FBG where id = @nroint  and folio = @folio ) begin
		...
		WHERE edo.NroInt = @nroint
		  and edo.Folio = @folio
		  and edo.Tipo = 'S';
		-- exec ksp_GDV_imprimir 1029190,3160263 ;
		...
		set @linea += 1; insert into ktb_lineas_FBG (id, folio, linea, dato) values ( @nroint, @folio, @linea, '4'); /* Tipo de Traslado. Obligatorio en Guia Despacho ; 1=venta|4=gratis|5=interno */
